...
import { getRow, getRows, executeQuery } from '@/lib/database/hybrid-connection';
  ...
  try {
    ...
    for (const team of allNFLTeams) {
      try {
        ...
        const existingTeam = await getRow('SELECT team_id FROM teams WHERE team_abbr = ?', [team.team_abbr]);
        ...
        if (existingTeam) {
          ...
          await executeQuery(`
            UPDATE teams SET 
              ...
              team_logo_url = ?
            WHERE team_abbr = ?
          `, [team.team_name, team.team_conference, team.team_division, team.team_color_primary, team.team_color_secondary, team.team_logo_url, team.team_abbr]);
          ...
          `, [team.team_id, team.team_name, team.team_abbr, team.team_conference, team.team_division, team.team_color_primary, team.team_color_secondary, team.team_logo_url]);
    ...
    // Get final count
    const teamsCount = await getRow('SELECT COUNT(*) as count FROM teams');
    
    console.log(`\n🎉 Successfully processed ${added + updated} teams!`);
    ...
    return NextResponse.json({ 
      ...
      message: `Successfully processed ${added + updated} teams. Total teams in database: ${teamsCount.count}`
    });
