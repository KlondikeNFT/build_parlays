...
async function syncNFLOfficialScheduleToTurso() {
  try {
    console.log('🔄 Syncing official NFL 2025 schedule to Turso database...');
    ...
    if (!fs.existsSync(schedulePath)) {
      throw new Error('NFL official schedule file not found. Please run parse-nfl-schedule.js and import-nfl-official-schedule.js first.');
    }
    ...
    // Create the official NFL schedule table in Turso
    console.log('🗄️ Creating nfl_official_schedule_2025 table in Turso...');
    ...
    // Clear existing data
    await client.execute('DELETE FROM nfl_official_schedule_2025');
    ...
    for (let i = 0; i < scheduleData.length; i += batchSize) {
      ...
      try {
        ...
        // Insert batch - process each game individually
        for (const gameValues of values) {
          await client.execute(`
    ...
    await client.execute('CREATE INDEX IF NOT EXISTS idx_nfl_official_schedule_date ON nfl_official_schedule_2025(game_date)');
    await client.execute('CREATE INDEX IF NOT EXISTS idx_nfl_official_schedule_week ON nfl_official_schedule_2025(week)');
    await client.execute('CREATE INDEX IF NOT EXISTS idx_nfl_official_schedule_home_team ON nfl_official_schedule_2025(home_team_id)');
    await client.execute('CREATE INDEX IF NOT EXISTS idx_nfl_official_schedule_away_team ON nfl_official_schedule_2025(away_team_id)');
    
    // Verify the data
    console.log('🔍 Verifying data in Turso...');
    const stats = await client.execute('SELECT COUNT(*) as count FROM nfl_official_schedule_2025');
    ...
    console.log('✅ NFL official schedule sync to Turso completed successfully!');
    
  } catch (error) {
    console.error('❌ Error syncing NFL official schedule to Turso:', error);
    throw error;
  }
}
...
if (require.main === module) {
  syncNFLOfficialScheduleToTurso()
    .then(() => {
      console.log('🎉 NFL official schedule sync to Turso completed!');
      process.exit(0);
    })
    .catch(error => {
...
module.exports = { syncNFLOfficialScheduleToTurso };
