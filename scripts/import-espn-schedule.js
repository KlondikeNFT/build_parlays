...
async function importESPNchedule() {
  try {
    console.log('📅 Importing ESPN NFL 2025 schedule into database...');
    
    // Check if the ESPN schedule file exists
    const schedulePath = path.join(__dirname, '..', 'data', 'espn-2025-schedule.json');
    if (!fs.existsSync(schedulePath)) {
      throw new Error('ESPN schedule file not found. Please run fetch-real-espn-schedule.js first.');
    }
    ...
    // Create the ESPN schedule table
    console.log('🗄️ Creating espn_schedule_2025 table...');
    db.exec(`
    ...
    // Clear existing data
    db.exec('DELETE FROM espn_schedule_2025');
    ...
    db.exec('CREATE INDEX IF NOT EXISTS idx_espn_schedule_date ON espn_schedule_2025(game_date)');
    db.exec('CREATE INDEX IF NOT EXISTS idx_espn_schedule_week ON espn_schedule_2025(week)');
    db.exec('CREATE INDEX IF NOT EXISTS idx_espn_schedule_home_team ON espn_schedule_2025(home_team_id)');
    db.exec('CREATE INDEX IF NOT EXISTS idx_espn_schedule_away_team ON espn_schedule_2025(away_team_id)');
    
    // Get statistics
    const stats = db.prepare('SELECT COUNT(*) as count FROM espn_schedule_2025').get();
    ...
    console.log('📊 ESPN Schedule Statistics:');
    ...
    db.close();
    console.log('✅ ESPN schedule import completed successfully!');
    
  } catch (error) {
    console.error('❌ Error importing ESPN schedule:', error);
    throw error;
  }
}
...
if (require.main === module) {
  importESPNchedule()
    .then(() => {
      console.log('🎉 ESPN schedule import completed!');
      process.exit(0);
    })
    .catch(error => {
...
module.exports = { importESPNchedule };
